...
#
# Native CPU-DRAM KV-cache offloading connector.
#
# Ported from upstream vLLM (commit c59a0eca4204, "[KV offload][4/N] Offloading
# KV connector", #22595) — the single-file version that drives the
# `vllm/v1/kv_offload/` infra present in this fork. Adapted to this fork's
# (ICMS-customized) KVConnectorBase_V1 / kv_offload APIs:
#   1. 3-arg ctor; OffloadingSpecFactory.create_spec(vllm_config, kv_cache_config)
#   2. worker reconstructs attn_backends (layer_name -> AttentionBackend) from
#      vllm_config, the way the model runner does, since this fork's
#      register_kv_caches() only receives kv_caches and spec.get_handlers()
#      requires attn_backends.
#   3. OffloadingWorker.get_finished() returns TransferResult dataclasses here
#      (upstream returned (job_id, success) tuples).
#   4. CachedRequestData exposes `resumed_req_ids` (a set) here, not a
#      per-request `resumed_from_preemption` bool list.
#
# Activated via:  --kv-offloading-size <GiB> --kv-offloading-backend native
# (which VllmConfig._post_init_kv_transfer_config maps to kv_connector=
#  "OffloadingConnector", cpu_bytes_to_use=<bytes>). NOT used by ICMS.
...
class OffloadingConnector(KVConnectorBase_V1):
    ...
    def __init__(self,
        ...
        if role == KVConnectorRole.SCHEDULER:
            ...
            self.connector_worker = OffloadingConnectorWorker(spec, vllm_config)
...
class OffloadingConnectorScheduler:
    ...
    def get_num_new_matched_tokens(
        ...
        Returns:
            (num tokens loadable beyond num_computed_tokens,
             True if the load is asynchronous).
        """
...
class OffloadingConnectorWorker:
    ...
    def __init__(self, spec: OffloadingSpec, vllm_config: VllmConfig):
        self.spec = spec
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        # This fork's spec.get_handlers() needs the per-layer AttentionBackend
        # (to read each layer's KV-cache shape/stride), but register_kv_caches
        # only gives us the tensors. Reconstruct the layer_name -> backend map
        # the same way the model runner does (get_layers_from_vllm_config +
        # layer.get_attn_backend()), restricted to the registered layers.
    ...
    def get_finished(self,
                     finished_req_ids: set[str]) -> tuple[set[str], set[str]]:
        """
        Returns ids of requests that finished asynchronous transfer:
        tuple of (sending/saving ids, recving/loading ids).
        """
        finished_sending = set()
        finished_recving = set()
        # This fork's OffloadingWorker.get_finished() returns TransferResult
        # dataclasses (upstream returned (job_id, success) tuples).
        for result in self.worker.get_finished():
            job_id, success = result.job_id, result.success
...
def yield_req_data(
    ...
    # cached requests. This fork exposes `resumed_req_ids` (a set) rather than
    # a per-request `resumed_from_preemption` bool list.
    cached_reqs = scheduler_output.scheduled_cached_reqs
    resumed = cached_reqs.resumed_req_ids
    for req_id, new_block_ids in zip(cached_reqs.req_ids,
                                     cached_reqs.new_block_ids):
        yield req_id, new_block_ids, (req_id in resumed)
